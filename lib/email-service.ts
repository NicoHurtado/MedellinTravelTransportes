...
export async function sendCambioEstadoEmail(
  ...
  const estadoTexto = {
    ES: {
      ...
      CONFIRMADA_PENDIENTE_ASIGNACION: 'Confirmada - Pendiente de Asignación',
    ...
    EN: {
      ...
      CONFIRMADA_PENDIENTE_ASIGNACION: 'Confirmed - Pending Assignment',
...
// ============================================
// TRIGGER 9: Tour Compartido Confirmado
// ============================================
export async function sendTourCompartidoConfirmationEmail(
  ...
  const content = language === 'ES' ? `
    ...
    <p>Hola <strong>${reserva.nombreCliente}</strong>,</p>
    <p>Gracias por reservar tu cupo en nuestro <strong>${serviceName}</strong>. Aquí tienes toda la información importante para tu viaje.</p>
    
    <div class="info-box">
      <h2 style="margin-top: 0; color: #D6A75D;">Punto de Encuentro</h2>
      <p style="font-size: 16px; font-weight: bold; margin-bottom: 5px;">📍 ${meetingPoint}</p>
      <p style="margin: 0;"><a href="${locationMapLink}" target="_blank" style="color: #3B82F6; text-decoration: underline;">Ver en Google Maps</a></p>
      
      <h2 style="margin-top: 15px; color: #D6A75D;">Hora de Encuentro</h2>
      <p style="font-size: 16px; font-weight: bold; margin: 0;">⏰ ${meetingTime}</p>
      
      <div style="background-color: #FFF3CD; padding: 10px; border-left: 4px solid #F59E0B; margin-top: 15px;">
        <p style="margin: 0; font-size: 14px; color: #856404;">
          <strong>IMPORTANTE:</strong> No existe servicio de recogida (pickup). Debes llegar por tus propios medios al punto de encuentro. Por favor sé puntual, el tour sale a la hora programada.
        </p>
      </div>
    </div>
    ...
    ${!reserva.esReservaAliado ? `
      ...
      <p style="color: #666; font-size: 14px;">Debes completar el pago para asegurar tu cupo.</p>
    ` : `
      <div class="info-box">
        <p><strong>Nota:</strong> El pago se realizará en efectivo al momento del servicio.</p>
      </div>
    ...
    <p>¡Te esperamos!</p>
  ` : `
    <h1>Shared Tour Booking Confirmed! 🚌</h1>
    <p>Hello <strong>${reserva.nombreCliente}</strong>,</p>
    <p>Thank you for booking your spot on our <strong>${serviceName}</strong>. Here is all the important information for your trip.</p>
    
    <div class="info-box">
      <h2 style="margin-top: 0; color: #D6A75D;">Meeting Point</h2>
      <p style="font-size: 16px; font-weight: bold; margin-bottom: 5px;">📍 ${meetingPoint}</p>
      <p style="margin: 0;"><a href="${locationMapLink}" target="_blank" style="color: #3B82F6; text-decoration: underline;">View on Google Maps</a></p>
      
      <h2 style="margin-top: 15px; color: #D6A75D;">Meeting Time</h2>
      <p style="font-size: 16px; font-weight: bold; margin: 0;">⏰ ${meetingTime}</p>
      
      <div style="background-color: #FFF3CD; padding: 10px; border-left: 4px solid #F59E0B; margin-top: 15px;">
        <p style="margin: 0; font-size: 14px; color: #856404;">
          <strong>IMPORTANT:</strong> There is no pickup service. You must arrive at the meeting point on your own. Please be punctual, the tour departs on schedule.
        </p>
      </div>
    </div>
    ...
    <div style="margin: 16px 0;">
      <p><strong>Code:</strong> ${reserva.codigo}</p>
      <p><strong>Date:</strong> ${formatDate(reserva.fecha, language)}</p>
      <p><strong>Passengers:</strong> ${reserva.numeroPasajeros} people</p>
    </div>
    ...
    ${!reserva.esReservaAliado ? `
      ...
      <p style="color: #666; font-size: 14px;">You must complete the payment to secure your spot.</p>
    ` : `
      <div class="info-box">
        <p><strong>Note:</strong> Payment will be made in cash at the time of service.</p>
      </div>
